classdef motionmodel
    ...
    methods(Static)
        ...
        function obj = camodel(T, sigma)
            %CAMODEL creates a 2D nearly constant acceleration model
            %INPUT: T: sampling time --- scalar
            %       sigma: standard deviation of motion noise --- scalar
            %OUTPUT:obj.d: object state dimension --- scalar
            %       obj.F: function handle return a motion transition
            ...
            %       obj.f: function handle return state prediction --- 6 x
            %       1 vector 
            %NOTE:  the motion model assumes that the state vector x
            %       consists of the following states: 
            %       px --- X-position
